...
class TCMS(object):
    ...
    def old_kinit(self, user, passwd):
    ...
    def filter_case_via_status(self, run_id, status):
        ...
        """
        case_ids = []
        cases = self.server.TestCaseRun.filter({
                        'run__run_id': run_id,
                        'case_run_status__name': status})
        case_ids = [case['case_id'] for case in cases]
        return case_ids
    ...
    def get_case_ids(self, plan_id, run_id, tags, priorities, is_tcms):
        ...
        if is_tcms:
            if run_id == '':
                ...
                # only collect idle case
                idle_case_ids = self.filter_case_via_status(run_id, 'IDLE')
                case_ids = list(set(case_ids) & set(idle_case_ids))
